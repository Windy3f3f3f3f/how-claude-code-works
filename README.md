...
[![Docs](https://img.shields.io/badge/Docs-21_chapters-orange?style=flat-square)](#专题深入)
...
这份 50 万行 TypeScript 的源码以快照形式在社区流出后，我们第一次能在一个真实的、跑在百万开发者机器上的生产级 Coding Agent 里，亲眼看到它到底长什么样。但 50 万行——从哪里开始读？
...
- 数十个内置工具同时存在，怎么协调？（如果所有工具上下文都给AI，那将直接爆炸）
...
1. 全链路流式输出 — 每生成一个 token 就立刻展示，不等模型全部想完。从 API 调用到终端渲染，整条链路都是流式的。
...
比如对话太长超出了上下文窗口，它悄悄压缩上下文、自动重试，不弹错误框让你手动处理。token 输出达到上限？自动从 8K 升级到 64K 再重试。整个 Agent 循环有 7 种不同的"继续"策略，每种对应一种故障恢复路径。

这就是为什么用 Claude Code 的时候很少遇到报错——不是没有错误，而是大部分都被内部消化了。

### 对话太长怎么办？—— 4 级渐进式压缩

这是整个系统里做得最细的一处设计。当上下文快要超限时，分 4 个级别逐步处理：
...
### 怎么防止 AI 执行危险操作？—— 7 层纵深防御

Claude Code 让 AI 直接在你电脑上跑命令，安全设计必须过硬。它不是靠一个"你确定吗？"对话框，而是搭建了 7 层防御体系：

1. 工作区信任 — 首次进入一个目录先确认信任；不信任就禁用该项目的所有自定义 Hook，挡住恶意仓库预埋脚本
2. 权限模式 — 不同信任级别，限制可执行的操作范围
3. 规则匹配 — 基于命令模式的 allow/deny/ask 列表
4. Bash 命令深度分析 — 这里最硬核：用语法树分析（不是正则匹配）拆解 Shell 命令的真实意图，包含 23 项静态安全检查，覆盖命令注入、环境变量泄露、特殊字符攻击等
5. 工具级安全 — 每个工具自带输入校验与专属安全逻辑（如文件编辑工具拦截危险路径）
6. 沙箱与隔离 — 进程级沙箱（macOS Seatbelt / Linux 命名空间）+ Git Worktree 文件隔离
7. 用户确认 — 危险操作弹确认框，与 Hook、LLM 分类器竞速；有 200ms 防抖保护，且用户一旦操作，人类意图永远优先

这 7 层任何一层拦住就不会执行，纵深防御。
...
| 4 | [工具系统](./docs/04-tool-system.md) | 数十个工具的注册与并发控制、MCP 6 种传输详解、连接状态机、OAuth 2.0 + PKCE 认证流程 |
...
| 12 | [权限与安全](./docs/11-permission-security.md) | 7 层纵深防御体系、tree-sitter AST 分析 + 23 项安全检查、竞速确认机制与 200ms 防误触 |
...
| 19 🔍 | [Dynamic Workflows：确定性脚本编排 agent 舰队](./docs/19-dynamic-workflows.md) | **快照之后·工具描述+strings+抓包**：把第 8 章"模型当协调器"换成确定性 JS 脚本 fan-out 几十上百 subagent；pipeline 无 barrier 的设计、schema 结构化输出、三道上限 + token 预算、worktree 隔离与断点续跑、把可靠性编排进流程的质量范式、ultraplan→ultracode 演进；文末附 Workflow 工具描述逐字节选 |
| 20 🔍 | [Agent Teams：对等组队与跨会话安全](./docs/20-agent-teams.md) | **快照之后·源码+strings**：从主从编排走到对等组队——SendMessage 寻址、Team = TaskList、共享记忆与 artifact；核心是那条"别的会话不携带用户权威"的跨会话规则（转发被拒动作＝权限洗白），接第 18 章 SOFT BLOCK 桶，加一道跨机 bridge 的 bypass-immune 守卫；文末附工具描述与规则逐字 |
| 21 🔍 | [后台 Agent 舰队：脱终端常驻与 daemon 监管](./docs/21-background-fleet.md) | **快照之后·strings 挑大梁**：`/bg` 把会话从终端摘下、关掉终端也照跑，`claude agents` 一张表管全部；近八十个 `tengu_bg_*` 拼出一台带健康检查/崩溃重生/孤儿收养/预热池的本地进程舰队监管器、on-demand daemon 与一键关停、子 agent 默认后台 + task-notification、完工自动 commit+push+draft PR；文末附关键字符串与事件族 |
...
| 源码总行数 | 512,000+ |
| TypeScript 文件 | 1,884 |
| 内置工具 | ~55（注册表 tools.ts，含 feature-gated） |
| 压缩流水线级数 | 4 级 |
| 权限防御层数 | 7 层 |
...
想理解核心原理？按顺序读 [主循环](./docs/02-agent-loop.md)、[上下文工程](./docs/03-context-engineering.md)、[工具系统](./docs/04-tool-system.md) 三章。

想自己造一个 AI Agent？先读 [最小必要组件](./docs/13-minimal-components.md)，然后跟着 [claude-code-from-scratch](https://github.com/Windy3f3f3f3f/claude-code-from-scratch) 的 13 章教程动手实现——~4300 行代码，每一步都对照源码讲解。

想定制 Claude Code？读 [Hooks 与可扩展性](./docs/06-hooks-extensibility.md)、[记忆系统](./docs/08-memory-system.md) 和 [技能系统](./docs/09-skills-system.md)。

关注安全？读 [权限与安全](./docs/11-permission-security.md) 和 [代码编辑策略](./docs/05-code-editing-strategy.md)。
...
- [x] 可观测性：Metrics 与 Trace（[#10](https://github.com/Windy3f3f3f3f/how-claude-code-works/issues/10) 提议）—— Claude Code 自身如何打点：OpenTelemetry 指标/事件导出、成本核算、会话 transcript 作为 turn 级 trace。已完成，见[第 16 章：可观测性](./docs/16-observability.md)
- [x] 自治闭环：`/goal`、`/loop` 与 cron 调度（v2.1.71 / v2.1.139）—— 设定完成条件后跨 turn 自主工作直到达成；定时/模型自定节奏的循环任务。已完成，见[第 17 章：自治与续跑](./docs/17-autonomy-goal-loop.md)，含 `/goal` 评估器与 `/loop` 命令的完整提示词原文 + 可复现逆向方法
- [x] Dynamic Workflows（v2.1.154 起，触发词 "ultracode"）—— 用一段编排脚本在后台指挥几十到几百个 agent，带 token 预算控制、断点续跑与 `/workflows` 监控面板。已完成，见[第 19 章：Dynamic Workflows](./docs/19-dynamic-workflows.md)，含 Workflow 工具描述逐字节选 + 可复现逆向方法
- [x] Auto Mode：权限进入分类器时代（v2.1.152 起免 opt-in）—— 从"规则 + 确认框"到 LLM 分类器实时判断每个动作放行/拦截，并能理解"别 push"这类口头边界。已完成，见[第 18 章：Auto Mode](./docs/18-auto-mode.md)，含分类器完整系统提示词原文 + 四规则桶 + 可复现逆向方法
- [x] Agent Teams 与跨会话安全（v2.1.166–178）—— `SendMessage` 组队协作；"跨会话消息不携带用户权威"的防提示注入设计。已完成，见[第 20 章：Agent Teams](./docs/20-agent-teams.md)，含 SendMessage / TeamCreate 工具描述与跨会话规则逐字 + 可复现逆向方法
- [x] 后台 Agent 舰队（v2.1.139–198）—— `/bg` 后台化、常驻 daemon、`claude agents` 全局面板、会话睡醒机制、完工自动 commit+push+draft PR、子 agent 默认后台运行。已完成，见[第 21 章：后台 Agent 舰队](./docs/21-background-fleet.md)，含近八十个 `tengu_bg_*` 事件族与关键字符串 + 可复现逆向方法
...
发现分析有误，或有更好的理解角度，欢迎提 Issue 和 PR 讨论。
...
| 日期 | 更新内容 |
|------|---------|
| 2026-07-07 | 新增第 20、21 章：Agent Teams（对等组队 + 跨会话不携带用户权威的防注入规则）与后台 Agent 舰队（`/bg` 脱终端、`claude agents` 面板、近八十个 `tengu_bg_*` 拼出的本地进程舰队监管器、完工自动 commit+PR）——本批两章用一段 Dynamic Workflow 脚本编排起草并经多轮 Codex 对抗审 |
| 2026-07-07 | 新增第 19 章：Dynamic Workflows（触发词 ultracode）——用确定性 JS 脚本编排几十上百个 subagent，讲 pipeline 无 barrier 的设计、并发/agent/预算三道上限、worktree 隔离与断点续跑，附 Workflow 工具描述节选 |
| 2026-07-06 | 新增第 18 章：Auto Mode——权限从"规则 + 确认框"进入 ML 分类器时代，含分类器完整系统提示词、四个自然语言规则桶、两段式分类与"别 push"的语义边界 |
| 2026-07-06 | 新增第 17 章：自治与续跑（/goal 与 /loop）——含 /goal 评估器与 /loop 命令的完整提示词原文，以及可复现的逆向方法 |
| 2026-07-06 | 第 16 章可观测性重写为清晰技术写作并去 AI 味；全书对 v2.1.88 源码快照做对抗式事实核查并修正；纵深防御口径统一为 7 层 |
| 2026-07-04 | 新增第 16 章：可观测性（Metrics 与 Trace，呼应 #10），开启"泄露快照之后的新功能"模块——对 goal/loop、Auto Mode、Dynamic Workflows 等快照后新能力做黑盒逆向 + 开源情报分析 |
| 2026-07-03 | 补充路线图 / TODO：规划快照（≈v2.1.6x）之后新功能的分析计划 |
| 2026-07-02 | 项目定位措辞重构，补充免责声明（教育性架构分析、非官方、不分发源码） |
| 2026-05-05 | 完善工具系统文档 |
| 2026-04-21 | 重组文档结构、更新 favicon，新增贡献者 |
| 2026-04-10 | 修复过宽的 Mermaid 图，改为纵向布局避免文字挤压 |

...
Claude Code 提供 **23+ 种 Hook 事件**，支持在工具调用、权限判定、会话管理等节点注入自定义逻辑。五种 Hook 类型（Command / Prompt / Agent / HTTP / Callback）覆盖从简单 Shell 脚本到企业 HTTP 服务的所有集成场景。

多 Agent 方面支持三种模式：
- **子 Agent**：通过 AgentTool fork 单个子任务
- **协调器**：主 Agent 只做编排，Worker 执行（研究 → 综合 → 实施 → 验证）
- **Swarm 团队**：命名 Agent 间对等信箱通信

记忆系统（4 种类型：user / feedback / project / reference）和技能系统（18+ 内置技能）共同使 Claude Code 成为跨会话学习的 Agent。

> 深入阅读：[Hooks 与可扩展性](./09-hooks-extensibility.md) | [多 Agent 架构](./10-multi-agent.md) | [记忆与技能系统](./11-memory-skills.md)
